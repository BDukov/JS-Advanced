class CarDealership {
    ...
    sellCar(model, desiredMileage) {
        ...
        if (mileageDelta <= 0) {
            ...
            sold.soldPrice = matchingModel.price * 0.90;
        }
    ...
    currentCar() {
        ...
        const result = ['-Available cars:', ...this.availableCars.map(c => `---${c.model} - ${c.horsepower} HP - ${c.mileage.toFixed(2)} km - ${c.price.toFixed(2)}$`)
        ];
